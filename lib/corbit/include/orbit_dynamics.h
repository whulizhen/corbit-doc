...
#define MAXNUM_FM_PARAM_EMP 800 // 864 params when  estimate 3 params in every 5 minutes
#define MAXNUM_PARAM_EMP_BASE 20
/// @brief maximum number of the parameters for the other force models (constant during the estimation)
#define MAXNUM_FM_PARAM_OHR 50
...
#define NSTATE_MAX  (6 + 36 + 6 * MAXNUM_FM_PARAM_TOTAL)
    ...
    void initialize_orbit_conifig_embed(ORBIT_DYN_CONFIG *ptr_orb_config);
    ...
    int compute_acc_Empirical(double mjd_utc, int idx_param_current[MAXNUM_FM_PARAM_EMP], PARAM_FM_EMP *fm_param, double *satpos, double *satvel, double *acc, double *dadp_eci);
    ...
    void test_orbit_simulation(char* orbit_dynamics_config_filename, char* output_orb_filename);
